...
const NewStudentModal: React.FC = () => {
    const [novoStudent, setNovoStudent] = useState<NewStudent>({ id: '', image_url: '', name: '', studentViews: [], frequency: 0 });
    ...
    const handleCloseStudentModal = () => {
        ...
        setNovoStudent({ id: '', image_url: '', name: '', studentViews: [], frequency: 0 });
    };
                    ...
                    {classrooms.map((classroom, key) => (
                        ...
                        <Grid item xs={4} key={classroom.id}>
                            <TextField
                                ...
                                type="number"
                                value={novoStudent.studentViews.find(input => input.classroomId === classroom.id)?.grade || 0}
                                onChange={(e) => handleGradeChange(classroom.id ?? -1, Number(e.target.value))}
                                fullWidth
                                sx={{ mb: 2 }}
                            />
                        </Grid>
