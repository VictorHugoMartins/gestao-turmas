...
const StudentViewContext = createContext<StudentViewContextProps | undefined>(undefined);

export const useStudentView = () => {
    const context = useContext(StudentViewContext);
    if (!context) {
        throw new Error('useStudentView must be used within an StudentViewProvider');
    }
    return context;
};

export const StudentViewProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
